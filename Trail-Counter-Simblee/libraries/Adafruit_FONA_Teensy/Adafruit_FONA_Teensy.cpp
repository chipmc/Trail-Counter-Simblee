...
// Comment out this section for Teensy to use hardware
/*#if (ARDUINO >= 100)
 #include "Arduino.h"
...
typedef char PROGMEM prog_char;  // Chip added to fix bug

#include "Adafruit_FONA_Teensy_Teensy.h"

#if ARDUINO >= 100
Adafruit_FONA_Teensy::Adafruit_FONA_Teensy(SoftwareSerial *ss, int8_t rst)
#else
Adafruit_FONA_Teensy::Adafruit_FONA_Teensy(NewSoftSerial *ssm, int8_t rst) 
#endif
*/
//  Use this his section for Teensey
...
#include "Adafruit_FONA_Teensy.h"
 
#if ARDUINO >= 100
Adafruit_FONA_Teensy::Adafruit_FONA_Teensy(HardwareSerial *ss, int8_t rst)
#else
Adafruit_FONA_Teensy::Adafruit_FONA_Teensy(NewSoftSerial *ss, int8_t rst) 
#endif
...
boolean Adafruit_FONA_Teensy::begin(uint16_t baudrate) {
...
boolean Adafruit_FONA_Teensy::getBattVoltage(uint16_t *v) {
  return sendParseReply(F("AT+CBC"), F("+CBC: "), v, ',', 2);
}

boolean Adafruit_FONA_Teensy::getADCVoltage(uint16_t *v) {
  return sendParseReply(F("AT+CADC?"), F("+CADC: 1,"), v);
}

/********* SIM ***********************************************************/

uint8_t Adafruit_FONA_Teensy::unlockSIM(char *pin)
{
...
uint8_t Adafruit_FONA_Teensy::getSIMCCID(char *ccid) {
   getReply("AT+CCID");
   // up to 20 chars
...
uint8_t Adafruit_FONA_Teensy::getNetworkStatus(void) {
  uint16_t status;
...
uint8_t Adafruit_FONA_Teensy::getRSSI(void) {
  uint16_t reply;
...
boolean Adafruit_FONA_Teensy::setAudio(uint8_t a) {
...
uint8_t Adafruit_FONA_Teensy::getVolume(void) {
  uint16_t reply;
...
boolean Adafruit_FONA_Teensy::setVolume(uint8_t i) {
  return sendCheckReply(F("AT+CLVL="), i, F("OK"));
}


boolean Adafruit_FONA_Teensy::playToolkitTone(uint8_t t, uint16_t len) {
  return sendCheckReply(F("AT+STTONE=1,"), t, len, F("OK"));
}

boolean Adafruit_FONA_Teensy::setMicVolume(uint8_t a, uint8_t level) {
  // 0 is headset, 1 is external audio
  if (a > 1) return false;
...
boolean Adafruit_FONA_Teensy::FMradio(boolean onoff, uint8_t a) {
  if (! onoff) {
...
boolean Adafruit_FONA_Teensy::tuneFMradio(uint16_t station) {
...
boolean Adafruit_FONA_Teensy::setFMVolume(uint8_t i) {
...
int8_t Adafruit_FONA_Teensy::getFMVolume() {
  uint16_t level;
...
int8_t Adafruit_FONA_Teensy::getFMSignalLevel(uint16_t station) {
  // Fail if FM station is outside allowed range.
...
boolean Adafruit_FONA_Teensy::PWM(uint16_t period, uint8_t duty) {
...
/********* CALL PHONES **************************************************/
boolean Adafruit_FONA_Teensy::callPhone(char *number) {
...
boolean Adafruit_FONA_Teensy::hangUp(void) {
  return sendCheckReply(F("ATH0"), F("OK"));
}

/********* SMS **********************************************************/

int8_t Adafruit_FONA_Teensy::getNumSMS(void) {
  uint16_t numsms;
...
// printouts!
boolean Adafruit_FONA_Teensy::readSMS(uint8_t i, char *smsbuff, 
...
#ifdef Adafruit_FONA_Teensy_DEBUG
...
boolean Adafruit_FONA_Teensy::sendSMS(char *smsaddr, char *smsmsg) {
...
boolean Adafruit_FONA_Teensy::deleteSMS(uint8_t i) {
    if (! sendCheckReply("AT+CMGF=1", "OK")) return -1;
  // read an sms
...
boolean Adafruit_FONA_Teensy::enableGPRS(boolean onoff) {
...
uint8_t Adafruit_FONA_Teensy::GPRSstate(void) {
  uint16_t state;
...
boolean Adafruit_FONA_Teensy::getGSMLoc(uint16_t *errorcode, char *buff, uint16_t maxlen) {
...
boolean Adafruit_FONA_Teensy::HTTP_GET_start(char *url, 
...
boolean Adafruit_FONA_Teensy::HTTP_GET_end(void) {
  flushInput();
...
inline int Adafruit_FONA_Teensy::available(void) {
  return mySerial->available();
}

inline size_t Adafruit_FONA_Teensy::write(uint8_t x) {
  return mySerial->write(x);
}

inline int Adafruit_FONA_Teensy::read(void) {
  return mySerial->read();
}

inline int Adafruit_FONA_Teensy::peek(void) {
  return mySerial->peek();
}

inline void Adafruit_FONA_Teensy::flush() {
  mySerial->flush();
}

void Adafruit_FONA_Teensy::flushInput() {
...
uint16_t Adafruit_FONA_Teensy::readRaw(uint16_t b) {
  uint16_t idx = 0;
...
uint8_t Adafruit_FONA_Teensy::readline(uint16_t timeout, boolean multiline) {
  uint16_t replyidx = 0;
...
uint8_t Adafruit_FONA_Teensy::getReply(char *send, uint16_t timeout) {
  flushInput();
...
uint8_t Adafruit_FONA_Teensy::getReply(const __FlashStringHelper *send, uint16_t timeout) {
  flushInput();
...
uint8_t Adafruit_FONA_Teensy::getReply(const __FlashStringHelper *prefix, char *suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t Adafruit_FONA_Teensy::getReply(const __FlashStringHelper *prefix, int32_t suffix, uint16_t timeout) {
  flushInput();
...
// Send prefix, suffix, and newline. Return response (and also set replybuffer with response).
uint8_t Adafruit_FONA_Teensy::getReply(const __FlashStringHelper *prefix, int32_t suffix1, int32_t suffix2, uint16_t timeout) {
  flushInput();
...
boolean Adafruit_FONA_Teensy::sendCheckReply(char *send, char *reply, uint16_t timeout) {
...
boolean Adafruit_FONA_Teensy::sendCheckReply(const __FlashStringHelper *send, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(send, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify FONA response matches reply parameter.
boolean Adafruit_FONA_Teensy::sendCheckReply(const __FlashStringHelper *prefix, char *suffix, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify FONA response matches reply parameter.
boolean Adafruit_FONA_Teensy::sendCheckReply(const __FlashStringHelper *prefix, int32_t suffix, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(prefix, suffix, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}

// Send prefix, suffix, and newline.  Verify FONA response matches reply parameter.
boolean Adafruit_FONA_Teensy::sendCheckReply(const __FlashStringHelper *prefix, int32_t suffix1, int32_t suffix2, const __FlashStringHelper *reply, uint16_t timeout) {
  getReply(prefix, suffix1, suffix2, timeout);
  return (strcmp_P(replybuffer, (prog_char*)reply) == 0);
}


boolean Adafruit_FONA_Teensy::parseReply(const __FlashStringHelper *toreply, 
...
boolean Adafruit_FONA_Teensy::sendParseReply(const __FlashStringHelper *tosend, 
